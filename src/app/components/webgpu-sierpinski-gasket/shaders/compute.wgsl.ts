const sierpinski_gasket_shader = (verticies: Array<Array<number>>, librate = false) => `
...
@group(0) @binding(2) var<storage, read_write> hist_atomic: array<atomic<i32>>;
@group(0) @binding(3) var<storage, read> frame_idx: f32;
...
const verts = array<vec3f,${verticies.length}>
(
    ${verticies.map(vert => {
        return `vec3<f32>(${vert[0]}, ${vert[1]}, ${vert[2]})`
    }).join(",\n")}
);
...
fn splat(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i = 0; i < 100; i++) {
        ...
        var vertex_id = i32(floor(hash_f() * ${verticies.length}.0 ));
        ...
        var mixed_point = ${librate == false ? "mix(vertex_p, p, 0.5);" : "mix(vertex_p, p, 0.65*abs(sin(frame_idx%1000/1000)));"}
...
`;
export default sierpinski_gasket_shader;